...
def main(window):
    ...
    p.setColor(QPalette.Window, QColor(145,145,145))
    window.setPalette(p)
    ...
    ####################couleur
    #0-2-4-8-16-32-64-128-256-564-1024-2048=12pcs
    ...
    def update():
        ...
        for i, v in enumerate(board):
            ...
            if v==0:
                pixmap=QPixmap('images/fond2.jpg')
                small_pix=pixmap.scaled(sizew,sizeh)
                labels[i].setPixmap(small_pix)
                
            elif v==2:
                pixmap=QPixmap('tuile/tuile2.png')
                small_pix=pixmap.scaled(sizew,sizeh)
                labels[i].setPixmap(small_pix)
                
            elif v==4:
                pixmap=QPixmap('tuile/tuile4.png')
                small_pix=pixmap.scaled(sizew,sizeh)
                labels[i].setPixmap(small_pix)
            elif v==8:
                pixmap=QPixmap('tuile/tuile8.png')
                small_pix=pixmap.scaled(sizew,sizeh)
                labels[i].setPixmap(small_pix)
            elif v==16:
                pixmap=QPixmap('tuile/tuile16.png')
                small_pix=pixmap.scaled(sizew,sizeh)
                labels[i].setPixmap(small_pix)
            elif v==32:
                pixmap=QPixmap('tuile/tuile32.png')
                small_pix=pixmap.scaled(sizew,sizeh)
                labels[i].setPixmap(small_pix)
            elif v==64:
                pixmap=QPixmap('tuile/tuile64.png')
                small_pix=pixmap.scaled(sizew,sizeh)
                labels[i].setPixmap(small_pix)
            elif v==128:
                pixmap=QPixmap('tuile/tuile128.png')
                small_pix=pixmap.scaled(sizew,sizeh)
                labels[i].setPixmap(small_pix)
            elif v==256:
                pixmap=QPixmap('tuile/tuile256.png')
                small_pix=pixmap.scaled(sizew,sizeh)
                labels[i].setPixmap(small_pix)
            elif v==512:
                pixmap=QPixmap('tuile/tuile512.png')
                small_pix=pixmap.scaled(sizew,sizeh)
                labels[i].setPixmap(small_pix)
            elif v==1024:
                pixmap=QPixmap('tuile/tuile1024.png')
                small_pix=pixmap.scaled(sizew,sizeh)
                labels[i].setPixmap(small_pix)
            elif v==2048:
                pixmap=QPixmap('tuile/tuile2048.png')
                small_pix=pixmap.scaled(sizew,sizeh)
                labels[i].setPixmap(small_pix)
                
                
            else:
                pixmap=QPixmap('evolutionpc/fond2.jpg')
                small_pix=pixmap.scaled(sizew,sizeh)
                labels[i].setPixmap(small_pix)
        window.update()
